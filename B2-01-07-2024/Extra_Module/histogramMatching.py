...
def match_histograms(src_image, ref_image):
    ...
    # test_a = tuple([0,1])
    src_hist_blue, bin_0 = np.histogram(src_b.flatten(), 256, [0,256]) # type: ignore
    src_hist_green, bin_1 = np.histogram(src_g.flatten(), 256, [0,256]) # type: ignore
    src_hist_red, bin_2 = np.histogram(src_r.flatten(), 256, [0,256]) # type: ignore
    ref_hist_blue, bin_3 = np.histogram(ref_b.flatten(), 256, [0,256]) # type: ignore
    ref_hist_green, bin_4 = np.histogram(ref_g.flatten(), 256, [0,256]) # type: ignore
    ref_hist_red, bin_5 = np.histogram(ref_r.flatten(), 256, [0,256]) # type: ignore
...
#     image_src = cv.imread(cv.samples.findFile(image_src_name))
#     image_ref = cv.imread(cv.samples.findFile(image_ref_name))
 
#     image_mask = None
#     if mask_provided:
#         image_mask = cv.imread(cv.samples.findFile(image_mask_name))
